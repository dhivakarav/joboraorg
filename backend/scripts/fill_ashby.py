"""G2: fill a REAL Ashby SPA apply form and validate — WITHOUT submitting.

Opens the live SPA, fills every field class (system fields, resume, UUID text
questions, Yes/No button questions), validates required fields, saves a
screenshot, then exits. It NEVER clicks submit and writes NO DB record.

Usage:
    python -m scripts.fill_ashby <Board> <job_id>
    python -m scripts.fill_ashby "https://jobs.ashbyhq.com/Ramp/<uuid>/application"
"""
import asyncio
import sys
from pathlib import Path

from app.adapters.ashby_fill import fill_form, screenshot
from app.adapters.ashby_inspect import APPLY_URL, UA, parse_ref
from app.logging_config import setup_logging

EVID = "/tmp/jobara_ashby_g2"
...
async def run(board, jid):
    from playwright.async_api import async_playwright
    url = APPLY_URL.format(board=board, job_id=jid)
    async with async_playwright() as p:
        b = await p.chromium.launch(headless=True)
        ctx = await b.new_context(user_agent=UA, viewport={"width": 1280, "height": 1700})
        page = await ctx.new_page()
        try:
            await page.goto(url, wait_until="domcontentloaded", timeout=35000)
            report = await fill_form(page, profile=TEST_PROFILE, resume_path=RESUME)
            await screenshot(page, f"{EVID}/{board}_{jid[:8]}_filled.png", report)
            return report
        finally:
...
def main():
    ...
    if len(args) == 1:
        ...
        if not ref:
            print("Could not parse board/job_id"); sys.exit(1)
        board, jid = ref
    elif len(args) == 2:
        board, jid = args
    else:
    ...
    report = asyncio.run(run(board, jid))
    ...
    print(f"Yes/No answered : {report.yesno_answered}")
    ...
    for f in report.fields:
        ...
        print(f"  [{mark}] {f.key[:34]:34} {f.kind:9} {req:9} {f.value_preview}")
